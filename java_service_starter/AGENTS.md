[根目录 AGENTS.md](../AGENTS.md) > **java_service_starter**
...
| `jss status` | `cmd_status()` | 调用 `get_service_status()` 查询端口/进程状态，展示 PID/环境/僵尸检测 |
| `jss envs` | `cmd_envs()` | 调用 `ProjectConfig.scan_envs()` 扫描环境文件 |
| `jss start` | `cmd_start()` | 调用 `start_service()` 自动编译+启动 |
| `jss build` | `cmd_build()` | 调用 `build_service()` 仅编译不启动（--force/--skip-deps） |
...
| `jss logs` | `cmd_logs()` | 读取 `logs/<service>-<env>/stdout.log` 尾部行 |
| `jss history` | `cmd_history()` | 从 `StateManager` 读取历史记录 |

### Python API（可被其他模块导入）

- `start_service(project, service, env, ...)` -- 自动编译+启动流程
- `build_service(project, service, *, force, skip_deps, state)` -- 编译+拷贝依赖（不启动），TDD/测试场景
- `stop_service(port)` -- 停止指定端口的服务
- `is_running(port)` -- 检查端口是否在监听
- `find_pid(port)` -- 查找占用端口的进程 PID
- `get_service_status(port)` -- 返回 (status, pid, elapsed)，支持僵尸检测
- `clear_service(project, service, state)` -- 删除 target 目录并清除编译记录
...
| `MavenConfig` | `models.py` | Maven 路径/配置，含 `build_compile_args()`（支持 goal 参数） |
...
当前**无测试目录和测试用例**。参见根 AGENTS.md 的测试策略章节获取建议优先级。
...
**Q: 自动编译的触发条件？**
A: `target/classes` 不存在或 `needs_compile()` 检测到源码变更时自动编译。`target/lib` 不存在时自动 `install` + `copy-dependencies`。

**Q: `jss build` 和 `jss start` 的区别？**
A: `build` 只编译+拷贝依赖，不启动 JVM 进程。`start` 在 `build` 的基础上继续启动服务。`build --skip-deps` 跳过依赖拷贝，适合 TDD 快速迭代。

**Q: classpath 构建策略？**
A: 优先使用 `target/lib`（jar 拷贝），回退到 `_resolve_maven_classpath`（Maven 本地仓库路径）。Maven 解析的顺序避免类冲突。
...
| 2026-05-15 09:30 | 功能增强 | 新增 `logs` 命令；自动编译机制；`status` 替代 `services`；`-T 1C` 并行构建；`_copy_dependencies` 依赖拷贝 |
