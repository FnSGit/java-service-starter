...
- `find_java_processes(port)` -- 查找匹配 server.port 的 Java 进程
- `clear_service(project, service, state)` -- 删除 target 目录并清除编译记录
- `_copy_dependencies(project_root, module, maven, java_config)` -- install + copy-dependencies 重建 target/lib
...
**Q: 为什么 `dependency:build-classpath` 在多模块项目中可能失败？**
A: reactor 内模块的 jar 不在本地仓库中。需要先 `mvn install -pl <module> -am` 安装到本地仓库。
...
java_service_starter/
  ...
  java_runner.py    # 核心运行时：进程管理、环境加载、classpath 构建、依赖拷贝
